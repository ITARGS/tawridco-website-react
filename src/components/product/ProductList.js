...
const ProductList = () => {
    ...
    // console.log("Filter Redux State -> ", filter);
    ...
    // console.log("total min price -> ", minmaxTotalPrice.total_min_price);
    // console.log("total max price -> ", minmaxTotalPrice.total_max_price);
    // console.log("Min price -> ", minmaxTotalPrice.min_price);
    // console.log("Max price -> ", minmaxTotalPrice.max_price);
    ...
    const getProductfromApi = () => {
        // console.log("getProductFrom Api called");
    ...
    useEffect(() => {
        if (city.city !== null && !selectedProduct.id) {
            ...
            if (filter.category_id === null && filter.brand_ids.length === 0 && filter.price_filter === null && filter.search === null && filter.section_id === null) {
                // console.log("Get Product From Api Called");

                getProductfromApi();

            }
            else {
                // dispatch({ type: ActionTypes.SET_FILTER_MIN_MAX_PRICE, payload: { min_price: minmaxTotalPrice.min_price, max_price: minmaxTotalPrice.max_price } });
                if (filter.price_filter?.min_price !== null && filter.price_filter?.max_price !== null) {
                    console.log("Filter api with price filter called");
                    ...
                    console.log("Filter api without price filter called");
                    filterProductsFromApi({
                        category_id: filter.category_id,
                        brand_ids: filter.brand_ids.toString(),
                        sort: filter.sort_filter,
    ...
    const filterProductsFromApi = async (filter) => {
        ...
        await api.getProductbyFilter(city.city.id, city.city.latitude, city.city.longitude, filter, cookies.get('jwt_token'))
            ...
            .then(result => {
                if (result.status === 1) {
                    // console.log("Filter Product By API Called");
                    setminmaxTotalPrice({
                        total_min_price: result.total_min_price,
                        total_max_price: result.total_max_price,
                        min_price: result.total_min_price,
                            ...
                            <>
                                ...
                                <ReactSlider
                                    ...
                                    onAfterChange={(values, index) => {
                                        ...
                                        setcurrPage(1);
                                        setoffset(0);
                                        dispatch({ type: ActionTypes.SET_FILTER_MIN_MAX_PRICE, payload: { min_price: values[0], max_price: values[1] } });
                                        closeCanvas.current.click();
                                    }}
                                />
                            </>
